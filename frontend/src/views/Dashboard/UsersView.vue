<template>
  <div class="bg-gray-100 p-4 rounded-lg shadow-md min-h-full min-w-full">
    <div class="p-5 bg-white p-4 rounded-lg shadow-md min-h-full min-w-full">
      ...
      <div class="m-3">
        <a href="#"
          class="inline-block bg-black text-white py-2 px-6 rounded-md cursor-pointer transition-colors duration-300 hover:bg-gray-400">
          <i class="bi bi-file-earmark-plus"></i> Add New User
        </a>
      </div>
      <div class="m-3 overflow-x-auto h-96">
        <table class="table-sticky  min-w-full text-left min-w-full table-auto">
          ...
          <tbody class="bg-grey-light w-full" style="height: 50vh;">
            <template v-for="(item, index) in user" :key="index">
              <tr class="w-full mb-4">
                ...
                <td class="m-5">
                  <a href="#"> <i class="bi bi-pencil-square mr-5" @click="openEditModal(item)"></i></a>
    ...
    <!-- Modal EDIT -->
    <div class="fixed inset-0 bg-gray-900 bg-opacity-50 flex items-center justify-center z-50" v-show="isModalOpen"
      @click="closeModal">
      <!-- Modal content -->
      <div class="bg-white p-8 rounded-lg shadow-md"  @click.stop="">
        <!-- Modal header -->
        ...
        <div class="mt-4 w-80">
          ...
          <input type="text" v-model="editedUser.name" class="mb-3 border border-gray-300 rounded-md p-2 w-full"
            placeholder="Name">
          <input type="text" v-model="editedUser.username" class="mb-3 border border-gray-300 rounded-md p-2 w-full"
            placeholder="Username">
          <input type="email" v-model="editedUser.email" class="mb-3 border border-gray-300 rounded-md p-2 w-full"
            placeholder="Email">
          <input type="password" v-model="editedUser.password" class="mb-3 border border-gray-300 rounded-md p-2 w-full"
    ...
    <div class="fixed inset-0 bg-gray-900 bg-opacity-50 flex items-center justify-center z-50"
      v-show="isModalDeleteOpen" @click="closeDeleteModal">
      <!-- Modal content -->
